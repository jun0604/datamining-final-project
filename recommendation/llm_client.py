...
def _fallback_result_summary(result):
    ...
    if not recs:
        return "입력 정보와 CSV 근거를 기준으로 추천 가능한 영양소를 찾지 못했습니다."
    ...
    return f"선택한 증상({symptoms})과 생활습관({lifestyles})이 symptom_nutrient.csv의 근거와 매칭되어 {nutrients}를 우선 추천했습니다."
...
아래 CSV 기반 추천 결과만 근거로 추천 이유를 3~5문장으로 작성하라.
