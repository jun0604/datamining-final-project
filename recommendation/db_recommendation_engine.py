...
from recommendation.db_symptom_engine import get_symptom_recommendations, get_symptom_rows
from recommendation.db_caution_engine import get_cautions, find_supplements_for_nutrients, get_stage_nutrient_status
...
from recommendation.db_repository import (
    load_table,
    contains_match,
    load_nutrient_alias_map,
...
def _fallback_parse_intake(intake_text, trace=None):
    ...
    med_df = load_table("medicine_info")
    supp_df = load_table("supplement_info")
...
def _nutrient_aliases(nutrient):
    ...
    # 2) DB nutrient_alias 테이블 별칭
    try:
        db_alias_map = load_nutrient_alias_map()
        for canonical, vals in db_alias_map.items():
            compact_vals = {_compact_text(v) for v in list(vals) + [canonical]}
            if key in compact_vals:
                aliases.update(vals)
                aliases.add(canonical)
                aliases.update(compact_vals)
...
def _collect_search_keywords(stage, symptoms, lifestyles, recommendations, supplements, medicines, caution_items):
    keywords = []
    keywords.extend(symptoms or [])
    keywords.extend(lifestyles or [])
    # 생활습관 문장의 핵심 단어를 추천 영양소보다 먼저 추가해 search_document 결과에 생활관리 근거가 포함되도록 한다.
    for item in lifestyles or []:
        text = str(item)
        if "한 번에" in text or "많은 양" in text:
            keywords.extend(["소량", "자주", "입덧", "수분"])
        if "식이섬유" in text:
            keywords.extend(["식이섬유", "섬유소", "수분", "변비"])
        if "철분" in text:
            keywords.extend(["철분", "빈혈", "비타민 C"])
    keywords.extend([r.get("nutrient", "") for r in recommendations or []])
    keywords.extend(supplements or [])
    keywords.extend(medicines or [])
    keywords.extend(caution_items or [])
    if stage:
        keywords.append(stage.get("stage_name", ""))
        keywords.append(f"임신 {stage.get('stage_name', '')}")
...
def run_recommendation(
    ...
    # search_document: 주의사항 근거 생성에 사용할 원문 chunk를 먼저 검색한다.
    # 최종 결과 페이지의 주의사항은 이 search_document 기반 근거를 포함한 result["cautions"]만 바탕으로 생성된다.
    search_keywords = _collect_search_keywords(stage, symptoms, lifestyles, recommendations, supplements, medicines, caution_items)
    search_documents = search_related_documents(search_keywords, limit=8)
    trace.step("search_document 기반 주의사항 원문 근거 검색", {
        "검색키워드": search_keywords,
        "검색결과수": len(search_documents),
        "검색결과": search_documents,
    })
    ...
    trace.step("주의사항 검사 입력 구성", {
        ...
        "search_document_근거수": len(search_documents),
    })
    ...
    # search_document: 주의사항 근거 생성에 사용된 원문 검색 결과를 최종 결과에도 보존
    result["search_documents"] = search_documents
    ...
    trace.step("LLM 추천이유 생성 입력", {
        ...
        "search_document_추가근거": search_documents,
    })
    ...
    trace.step("LLM 주의사항 생성 입력", {
        ...
        "주의사항근거": cautions,
        "search_document_추가근거수": len(result.get("search_documents", [])),
    })
