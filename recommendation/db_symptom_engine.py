...
from recommendation.db_repository import load_table, split_items, unique_list
...
def _row_stage_match(row, stage_name):
    ...
    if not stage_name:
        return True
    # DB 규칙에 임신단계가 비어 있으면 공통 규칙으로 본다.
...
def get_symptom_rows(symptoms, trace=None):
    if not symptoms:
        ...
        return []
    df = load_table("symptom_nutrient")
...
def get_symptom_recommendations(symptoms, lifestyles=None, stage_name=None, trace=None):
    ...
    for row_index, row in df.iterrows():
        ...
        for nutrient in nutrients:
            ...
            score = 0.0
            score_breakdown = []
            base_score = _to_float(row.get("base_score", ""), 1.0)
            weight_delta = _to_float(row.get("weight_delta", ""), 0.0)
            rule_source = str(row.get("rule_source", "")).strip()

            if symptom_hit:
                triggers.append(symptom)
                add = base_score if rule_source != "lifestyle_weight_rule" else 0.0
                score += add
                score_breakdown.append({"항목": "증상일치", "값": symptom, "가산점": add})
            if lifestyle_hit:
                triggers.append(lifestyle)
                add = weight_delta if rule_source == "lifestyle_weight_rule" else 0.7
                score += add
                score_breakdown.append({"항목": "생활습관일치", "값": lifestyle, "가산점": add})

            # DB의 생활습관 가중치 규칙은 생활습관 선택이 있을 때만 적용한다.
            if rule_source == "lifestyle_weight_rule" and not lifestyle_hit:
                continue
            if score <= 0:
                continue
