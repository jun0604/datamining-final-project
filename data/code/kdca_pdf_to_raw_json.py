# -*- coding: utf-8 -*-
"""
kdca_pdf_to_raw_json.py

로컬 실행용 KDCA PDF -> raw JSON 변환 스크립트
...
│  └─ kdca_pdf_to_raw_json.py
├─ raw/
│  ├─ 정상임신관리(임신의 진단과 관리) _ 국가건강정보포털 _ 질병관리청.pdf
│  └─ 식이영양(임산부) _ 국가건강정보포털 _ 질병관리청.pdf
├─ processed/
└─ log/

실행 위치:
python data/code/kdca_pdf_to_raw_json.py

출력:
data/raw/*_raw.json
data/log/kdca_pdf_to_raw_json_YYYYMMDD_HHMMSS.log
"""
...
PDF_PATHS = find_target_pdfs(RAW_DIR)
